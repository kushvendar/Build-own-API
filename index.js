...
app.get("/jokes/:id",(req,res)=>{
  ...
  const joke=jokes.find((joke)=>jokes.id===id);
  res.json(joke);
})
//3. GET a jokes by filtering on the joke type
app.get("/jokes/filter",(req,res)=>{
  var i=req.params.type;
  const joke=jokes.filter((joke)=>jokes.jokeType === i);
  res.json(joke);
})
//4. POST a new joke
...
app.put("/jokes/:id",(req,res)=>{
  ...
  const idx=jokes.findIndex((joke)=>jokes.id===id);
  jokes[idx]=newjoke;
  res.json(jokes[idx]);
})
//6. PATCH a joke
...
res
.status(404)
.json({error:`joke with the ${id} not found`})
}
...
app.delete("/all",(req,res)=>{
  ...
  if(key===masterKey){
    ...
    res
    .sendStatus(404)
    .json({error:`api key did not match no data deleted`})
  }
